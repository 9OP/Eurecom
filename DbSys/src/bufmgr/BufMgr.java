...
class FrameDesc implements GlobalConst {
    ...
    /**
     * Decrements the pin count of a frame when the page is unpinned. If the pin count is equal to
     * or less than zero, the pin count will be zero.
     *
...
public class BufMgr implements GlobalConst {
    ...
    // the 3rd argument'int lastRef' couldn't be added because the code BMTest wouldn't run
    {
        ...
        if (replacerArg == null) {
            ...
            if (replacerArg.compareTo("Clock") == 0) {
                ...
                /*
                 * Assumed the value of lastRef is 2 since the argument could not be added to the
                 * constructor of BufMgr
                 */
    ...
    /**
     * Check if this page is in buffer pool, otherwise find a frame for this page, read in and pin
     * it. Also write out the old page if it's dirty before reading if emptyPage==TRUE, then
     * actually no read is done to bring the page in.
     *
    ...
    public void pinPage(PageId pin_pgid, Page page, boolean emptyPage)
        ...
        if (replacer.name() == "LRUK") {
            ...
            // the pageid of the page that is currently referenced to generate and update the
            // history
        ...
        if (frameNo < 0) { // Not in the buffer pool
            ...
            if (replacer.name() == "LRUK") {
                // we tell LRUK that the current page is in the buffer
                // to avoid updating time with correlation ref period
                ((LRUK) replacer).page_in_buffer = 1;
            }
            page.setpage(bufPool[frameNo]);
            replacer.pin(frameNo);

            if (replacer.name() == "LRUK") {
                ((LRUK) replacer).page_in_buffer = 0;
            }
    ...
    /**
     * To unpin a page specified by a pageId. If pincount>0, decrement it and if it becomes zero,
     * put it in a group of replacement candidates. if pincount=0 before this call, return error.
     *
    ...
    /**
     * Call DB object to allocate a run of new pages and find a frame in the buffer pool for the
     * first page and pin it. If buffer is full, ask DB to deallocate all these pages and return
     * error (null if error).
     *
    ...
    public PageId newPage(Page firstpage, int howmany) throws BufferPoolExceededException,
            HashOperationException, ReplacerException, HashEntryNotFoundException,
            InvalidFrameNumberException, PagePinnedException, PageUnpinnedException,
            PageNotReadException, BufMgrException, DiskMgrException, IOException {
        int i;
